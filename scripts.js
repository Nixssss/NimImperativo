...
function nextSlide() {
    if (!showAll && currentSlide < slides.length - 1) {
        currentSlide++;
...
function toggleShowAll() {
    if (!showAll) {
        ...
        currentSlide = slides.length; // Para desativar navegação individual
    } else {
        // Volta a mostrar uma slide de cada vez
        currentSlide = 0; // Reinicia a partir da primeira slide
        showSlide(currentSlide);
        slides.forEach((slide, i) => {
            if (i !== 0) slide.classList.remove('active'); // Esconde todas as outras slides
        });
